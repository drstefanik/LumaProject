...
type ReportRecord = { id: string; fields: Record<string, unknown>; createdTime?: string };
...
export async function POST(
  ...
  try {
    const normalized = normalizeReportId(reportId).trim();
    ...
    const workerUrl = `${request.nextUrl.origin}/api/pdf-worker`;
    ...
    const pdfBytes = Buffer.from(await workerRes.arrayBuffer());
    ...
    try {
      await updateReportByRecordId(tableName, report.id, {
        PDFUrl: blobUrl,
        PDFStatus: "final",
